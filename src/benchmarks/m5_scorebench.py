"""M5 scorebench CLI wrapper.

Provides a CLI interface for running M5 scoring throughput benchmarks.
"""
...
class BenchConfig:
    """Configuration for M5 scorebench."""
...
def run_scorebench(config: BenchConfig) -> dict[str, Any]:
    """Run M5 scorebench and return report."""
    ...
    def eval_fn(p_chunk: Any, f_chunk: Any) -> Any:
        kernel_launches[0] += 1
        base = p_chunk[:, :1]
        if p_chunk.shape[1] > 2:
            base = base + p_chunk[:, 1:2] * 0.0005 - p_chunk[:, 2:3] * 0.00025
        return base * (f_chunk[None, :] + 1.0)

    evaluator = ChunkedEvaluator(backend, eval_fn, chunking=config.chunking_policy())

    def run_once() -> Any:
        prediction = evaluator.evaluate(params, freqs)
        scores = ((prediction - target[None, :]) ** 2).mean(axis=1)
        return scores.sum()
    ...
    total_scores = config.n_params * config.repeats
    element_throughput = total_elements / elapsed if elapsed > 0 else float("inf")
    score_throughput = total_scores / elapsed if elapsed > 0 else float("inf")

    pass_elements = element_throughput >= config.min_elements_per_s
    pass_scores = score_throughput >= config.min_scores_per_s
    status = "pass" if (pass_elements and pass_scores) else "fail"
    ...
    return {
        ...
        "throughput_elements_per_s": element_throughput,
        "throughput_scores_per_s": score_throughput,
        ...
        "thresholds": {
            "min_elements_per_s": config.min_elements_per_s,
            "min_scores_per_s": config.min_scores_per_s,
        },
    }


def main(argv: Sequence[str] | None = None) -> int:
    """Run M5 scorebench CLI."""
    ...
    config = BenchConfig(
        ...
        min_scores_per_s=args.min_scores_per_s,
    )

    try:
        report = run_scorebench(config)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="M5 scorebench")
    ...
    parser.add_argument("--min-scores-per-s", type=float, default=1.0)
