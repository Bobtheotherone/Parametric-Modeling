...
class SParameterData:
    ...
    def __post_init__(self) -> None:
        ...
        if self.s_parameters.shape != expected_shape:
            raise ValueError(
                f"s_parameters shape {self.s_parameters.shape} does not match "
    ...
    while idx < len(all_tokens):
        ...
        if n_ports == 1:
            ...
            for out_idx, in_idx in order:
                val1 = float(all_tokens[idx])
                val2 = float(all_tokens[idx + 1])
                s_matrix[out_idx][in_idx] = _convert_to_complex(
                    val1, val2, options.parameter_format
                )
                idx += 2
        else:
...
def _write_touchstone_content(
    ...
    f.write(f"! Generated by formula_foundry.em.touchstone\n")
    ...
    for f_idx in range(data.n_frequencies):
        ...
        if n_ports == 1:
            ...
            val1, val2 = _complex_to_format(
                data.s_parameters[f_idx, 0, 0], options.parameter_format
            )
            ...
            for out_idx, in_idx in order:
                val1, val2 = _complex_to_format(
                    data.s_parameters[f_idx, out_idx, in_idx], options.parameter_format
                )
...
def validate_with_skrf(data: SParameterData, tolerance_db: float = 0.01) -> bool:
    ...
    try:
        ...
        raise ImportError(
            "scikit-rf is required for validation. Install with: pip install scikit-rf"
        ) from e
    ...
    if np.any(np.isnan(network.s)) or np.any(np.isinf(network.s)):
        return False
